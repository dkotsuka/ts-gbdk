...
test("bool → uint8_t param", () =>
  assertC("function check(flag: bool): void {}\nfunction updateFrame(): void {}",
...
test("binary +", () =>
  assertC("function f(): u8 { return 1 + 2; }\nfunction updateFrame(): void {}",
    ["return (1 + 2);"]));

test("binary &", () =>
  assertC("function f(a: u8, b: u8): u8 { return a & b; }\nfunction updateFrame(): void {}",
    ["return (a & b);"]));

test("binary comparison ===", () =>
  assertC("function f(a: u8): bool { return a === 0; }\nfunction updateFrame(): void {}",
    ["return (a == 0);"]));
...
test("prefix !", () =>
  assertC("function f(a: bool): bool { return !a; }\nfunction updateFrame(): void {}",
    ["return !(a);"]));
...
test("boolean literal true → 1", () =>
  assertC("function f(): bool { return true; }\nfunction updateFrame(): void {}",
    ["return 1;"]));

test("boolean literal false → 0", () =>
  assertC("function f(): bool { return false; }\nfunction updateFrame(): void {}",
    ["return 0;"]));
...
test("if without else", () =>
  assertC("function f(x: u8): void { if (x > 0) { x = 0; } }\nfunction updateFrame(): void {}",
    ["if ((x > 0)) {", "(x = 0);"]));

test("if with else", () =>
  assertC("function f(x: u8): u8 { if (x > 0) { return x; } else { return 0; } }\nfunction updateFrame(): void {}",
    ["if ((x > 0)) {", "else {", "return 0;"]));

test("while loop", () =>
  assertC("function f(x: u8): void { while (x > 0) { x = x - 1; } }\nfunction updateFrame(): void {}",
    ["while ((x > 0)) {", "(x = (x - 1));"]));

test("for loop", () =>
  assertC("function f(): void { for (let i: u8 = 0; i < 10; i++) {} }\nfunction updateFrame(): void {}",
    ["for (uint8_t i = 0; (i < 10); (i)++)"]));

test("local const var", () =>
  assertC("function f(): void { const n: u8 = 5; }\nfunction updateFrame(): void {}",
    ["uint8_t n = 5;"]));
...
test("updateFrame is called in _update", () =>
  assertC("function updateFrame(): void {}",
    ["test_updateFrame();", "void test_update(void)"]));
...
test("class raises TSGBDK014", () =>
  assertDiag("class Foo {}", "TSGBDK014"));
